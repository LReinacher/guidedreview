...
Local server (`server/createServer.ts`) + the same overlay. Same engine, same `ProviderClient` contract.

A **coding agent** (`codingAgents/`) is how the CLI reviews without an API key of its own. Two shapes:

- **Runs locally** — the adapter sets `reviewClient`, and reviews are generated by running the agent's binary (`claude -p`, see `providers/claudeCli.ts`). It authenticates itself, so `AgentAuth.secret` is empty. Do not read Claude Code's keychain or `.credentials.json`: a subscription login is an OAuth session `api.anthropic.com` rejects with 429.
- **Lends a token** — Codex/Grok hand back a secret we send to the provider HTTP API.

`canGenerateReview` (not `settings.apiKey`) decides whether a review can run; `publicSettings().ready` is the same signal for the browser UI.
