import type { ProviderClient, ProviderId } from "@guided-review/core";
...
export interface AgentAuth {
  provider: ProviderId;
  model?: string;
  /** Empty for `kind: "cli"` — the agent's binary holds the credential. */
  secret: string;
  /** `cli` means the review is generated by running the agent, not by us. */
  kind: "api_key" | "oauth" | "cli";
...
export interface CodingAgentAdapter {
  ...
  provider: ProviderId;
  /**
   * Set when reviews run through the agent's own binary instead of the
   * provider HTTP API. Such an agent needs no API key of ours.
   */
  reviewClient?: ProviderClient;
